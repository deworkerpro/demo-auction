...
api-clear:
	docker run --rm --volume "${PWD}/api":/app --workdir /app alpine:3.22 sh -c 'rm -rf var/cache/* var/log/* var/test/*'

api-init: api-permissions api-deps-install api-wait-db api-migrations api-fixtures

api-permissions:
	docker run --rm --volume "${PWD}/api":/app --workdir /app alpine:3.22 chmod 777 var/cache var/log var/test
...
frontend-clear:
	docker run --rm --volume "${PWD}/frontend":/app --workdir /app alpine:3.22 sh -c 'rm -rf .ready build'
...
frontend-ready:
	docker run --rm --volume "${PWD}/frontend":/app --workdir /app alpine:3.22 touch .ready
...
cucumber-clear:
	docker run --rm --volume "${PWD}/cucumber":/app --workdir /app alpine:3.22 sh -c 'rm -rf var/*'
...
testing-init:
	...
	docker compose -f docker-compose-testing.yml run --rm api-php-cli wait-for-it api-postgres:5432 -t 60
	docker compose -f docker-compose-testing.yml run --rm api-php-cli php bin/app.php migrations:migrate --no-interaction
	docker compose -f docker-compose-testing.yml run --rm testing-api-php-cli php bin/app.php fixtures:load --no-interaction
	sleep 15

testing-smoke:
	docker compose -f docker-compose-testing.yml run --rm cucumber-node-cli yarn smoke-ci

testing-e2e:
	docker compose -f docker-compose-testing.yml run --rm cucumber-node-cli yarn e2e-ci

testing-down-clear:
	docker compose -f docker-compose-testing.yml down --volumes --remove-orphans --timeout=1
