...
api-clear:
	docker run --rm -v ${PWD}/api:/app -w /app alpine:3.22 sh -c 'rm -rf var/cache/* var/log/* var/test/*'

api-init: api-permissions api-deps-install api-wait-db api-migrations api-fixtures

api-permissions:
	docker run --rm -v ${PWD}/api:/app -w /app alpine:3.22 chmod 777 var/cache var/log var/test
...
frontend-clear:
	docker run --rm -v ${PWD}/frontend:/app -w /app alpine:3.22 sh -c 'rm -rf .ready build'
...
frontend-ready:
	docker run --rm -v ${PWD}/frontend:/app -w /app alpine:3.22 touch .ready
...
cucumber-clear:
	docker run --rm -v ${PWD}/cucumber:/app -w /app alpine:3.22 sh -c 'rm -rf var/*'
