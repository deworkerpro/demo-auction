...
frontend-ready:
	docker run --rm --volume "${PWD}/frontend":/app --workdir /app --user ${UID}:${GID} alpine:3.22 touch .ready
...
testing-build-cucumber:
	docker --log-level=debug buildx build --pull \
		--build-arg GID=${GID} --build-arg UID=${UID} \
		--tag=${REGISTRY}/auction-cucumber-node-cli:${IMAGE_TAG} --file=cucumber/docker/testing/node/Dockerfile cucumber
...
testing-down-clear:
	docker compose -f compose-testing.yml down --volumes --remove-orphans --timeout=1
