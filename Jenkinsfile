    ...
    stages {
        stage('Init') {
            steps {
                ...
                sh 'make init-ci'
                sh 'docker compose images | sed \'s/[0-9]\\+ [a-z]\\+ ago//\' > .docker-images-after'
                script {
                    DOCKER_DIFF = sh(
                        returnStdout: true,
        ...
        stage ('Prod') {
            ...
            steps {
                ...
                withCredentials([
                    ...
                    string(credentialsId: 'OAUTH_YANDEX_CLIENT_SECRET', variable: 'OAUTH_YANDEX_CLIENT_SECRET'),
                    string(credentialsId: 'OAUTH_MAILRU_CLIENT_ID', variable: 'OAUTH_MAILRU_CLIENT_ID'),
                    string(credentialsId: 'OAUTH_MAILRU_CLIENT_SECRET', variable: 'OAUTH_MAILRU_CLIENT_SECRET'),
                    string(credentialsId: 'BACKUP_AWS_ACCESS_KEY_ID', variable: 'BACKUP_AWS_ACCESS_KEY_ID'),
                    string(credentialsId: 'BACKUP_AWS_SECRET_ACCESS_KEY', variable: 'BACKUP_AWS_SECRET_ACCESS_KEY'),
